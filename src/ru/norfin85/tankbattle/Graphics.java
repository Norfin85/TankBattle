...
import static ru.norfin85.tankbattle.TankBattle.battleLog;
...
public class Graphics {
    ...
    private static final JFrame logFrame = new JFrame("Battle log"); //Создаем окно
    ...
    private static final JPanel bottomPanel = new JPanel(); //нижняя панель-обертка
    ...
    private static final JTextPane battleLog = new JTextPane();
    private static final JScrollPane battleLogJsp = new JScrollPane(battleLog);
    ...
    private static final JButton logButton = new JButton("Лог боя"); //добавляем кнопку
    //создаем танки
    ...
    private static StyledDocument doc, doc1;
    ...
    public static void createAndShowGUI() {
        ...
        logFrame.pack(); //оптимизируем окно под размеры элементов
        logFrame.setVisible(false); //делаем окно невидимым
        logFrame.setResizable(false); //Запрещаем изменять размеры окна
        logFrame.setSize(800, 600); //Устанавливаем размер окна
        ...
        battleList.setPreferredSize(new Dimension(380, 410)); //размер панели
        battleList.setBorder(border); //создаем рамку
        textPanel.setBorder(new EmptyBorder(5, 5, 5, 5)); //создаем отступы
        jsp.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
        jsp.setPreferredSize(new Dimension(380, 410));
        doc1 = battleLog.getStyledDocument();
        simpleAttributeSet = new SimpleAttributeSet();
        StyleConstants.setAlignment(simpleAttributeSet, StyleConstants.ALIGN_LEFT);
        StyleConstants.setFontSize(simpleAttributeSet, 12);
        StyleConstants.setFontFamily(simpleAttributeSet, "Times New Roman, cursive");
        simpleAttributeSet.addAttribute(StyleConstants.CharacterConstants.Foreground, new Color(112, 126, 173));
        doc1.setParagraphAttributes(0, doc1.getLength(), simpleAttributeSet, false);
        battleLog.setText("");
        battleLog.setEditable(false);
        battleLogJsp.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
        ...
        button.setPreferredSize(new Dimension(150, 50)); //размер кнопки
        button.setAlignmentX(Component.LEFT_ALIGNMENT); //центральное размещение
        button.addActionListener(new PushingListener());
        logButton.setPreferredSize(new Dimension(100, 50)); //размер кнопки
        logButton.setAlignmentX(Component.RIGHT_ALIGNMENT); //центральное размещение
        logButton.addActionListener(new PushingListener());
        bottomPanel.add(button); //добавление в окно кнопку
        bottomPanel.add(logButton);
        frame.add(mainPanel); //добавляем в окно главную панель
        frame.add(bottomPanel);
        logFrame.add(battleLogJsp);
    }
    ...
    public static void startBattle() {
        ...
        battleLog.setText(battleLog.getText() + battleList.getText());
        battleList.repaint();
        if (getTanks() == 0) {
            Graphics.setButton("Бой закончен", false);
            updateFrame();
        }
    }
